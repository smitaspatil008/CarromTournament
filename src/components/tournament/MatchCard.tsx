...
function TeamLogoCircle({ team }: { team: Team }) {
  return (
    <div className="w-9 h-9 sm:w-12 sm:h-12 rounded-lg sm:rounded-xl flex items-center justify-center text-white font-display font-bold text-sm sm:text-lg" style={{ background: team.color }}>
      {team.logo}
    </div>
  );
}
...
export default function MatchCard({ match, teamA, teamB, playerA, playerB, compact }: Props) {
  ...
  return (
    ...
    >
      {/* Top bar */}
      <div className={`px-3 sm:px-4 py-1.5 sm:py-2 flex items-center justify-between ${isLive ? 'bg-red-50' : 'bg-gray-50'}`}>
        <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
          {isLive && <LiveBadge size="sm" />}
          {!isLive && (
            <span className={`text-[10px] sm:text-xs font-medium px-1.5 sm:px-2 py-0.5 rounded-full flex-shrink-0 ${isDone ? 'bg-green-50 text-green-600' : 'bg-blue-50 text-blue-600'}`}>
              {isDone ? 'Completed' : 'Upcoming'}
            </span>
          )}
          <span className="text-[10px] sm:text-xs text-gray-500 capitalize truncate">{match.game} · {match.round}</span>
        </div>
      ...
      <Link to={`/match/${match.id}`} className="block p-3 sm:p-4 hover:bg-gray-50 transition-colors">
        <div className="flex items-center gap-2 sm:gap-3">
          {/* Team A */}
          <div className="flex-1 flex items-center gap-2 sm:gap-3 min-w-0">
            <TeamLogoCircle team={teamA} />
            <div className="min-w-0">
              <div className="font-semibold text-xs sm:text-sm truncate" style={{ color: match.winner === teamA.id ? teamA.color : '#111827' }}>
                {teamA.name}
              </div>
              {playerA && <div className="text-[10px] sm:text-xs text-gray-500 truncate">{playerA.name}</div>}
          ...
          <div className="flex-1 flex items-center justify-end gap-2 sm:gap-3 min-w-0">
            <div className="min-w-0 text-right">
              <div className="font-semibold text-xs sm:text-sm truncate" style={{ color: match.winner === teamB.id ? teamB.color : '#111827' }}>
                {teamB.name}
              </div>
              {playerB && <div className="text-[10px] sm:text-xs text-gray-500 truncate">{playerB.name}</div>}
