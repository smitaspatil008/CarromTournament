...
function SequencePlayoffBracket({ matches, teams }: { matches: any[]; teams: any[] }) {
  ...
  return (
    <div className="surface rounded-2xl p-6">
      <h3 className="font-display font-bold text-lg gradient-text mb-6">Sequence Playoffs</h3>
      <div className="flex flex-col sm:flex-row items-start gap-6 overflow-x-auto">
        {/* Semi Finals */}
        <div className="flex-1 min-w-[200px]">
          <div className="text-center mb-3">
            <span className="text-xs font-bold px-3 py-1 rounded-full text-white" style={{ background: 'linear-gradient(135deg,#7C3AED,#F97316)' }}>
              Semi Finals
            </span>
          </div>
              ...
              return (
                <Link key={matchId} to={`/match/${matchId}`}>
                  <motion.div whileHover={{ scale: 1.02 }}
                    className={`rounded-xl overflow-hidden text-xs cursor-pointer ${isLive ? 'ring-2 ring-red-500' : ''}`}
                    ...
                    <div className="px-1 py-0.5 text-center text-[9px] text-muted border-b" style={{ borderColor: 'var(--color-border)' }}>{label}</div>
                    {[{team: tA, score: match.scoreA, winner: match.winner === tA?.id}, {team: tB, score: match.scoreB, winner: match.winner === tB?.id}].map((row, i) => (
                      <div key={i} className={`flex items-center justify-between px-3 py-2 ${i === 0 ? 'border-b' : ''} ${row.winner ? 'font-bold' : ''}`}
                        style={{ borderColor: 'var(--color-border)', background: row.winner ? `${row.team?.color}15` : 'transparent' }}>
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="truncate max-w-[90px]" style={{ color: row.winner ? row.team?.color : 'var(--color-text)' }}>{row.team?.name ?? 'TBD'}</span>
                        </div>
                        <span className="font-bold ml-2 tabular-nums" style={{ color: row.winner ? row.team?.color : 'var(--color-text)' }}>
        ...
        <div className="flex items-center self-center sm:mt-8">
          <div className="w-8 h-px hidden sm:block" style={{ background: 'linear-gradient(90deg,#7C3AED,#F97316)' }} />
          <div className="w-0 h-0 border-t-4 border-t-transparent border-b-4 border-b-transparent border-l-8 hidden sm:block" style={{ borderLeftColor: '#F97316' }} />
        </div>

        {/* Final */}
        <div className="flex-1 min-w-[200px]">
          <div className="text-center mb-3">
            <span className="text-xs font-bold px-3 py-1 rounded-full text-white" style={{ background: 'linear-gradient(135deg,#F97316,#fbbf24)' }}>
              Final
            </span>
          </div>
          {fin && (
            <div className="mt-6">
              <Link to={`/match/${fin.id}`}>
                <motion.div whileHover={{ scale: 1.02 }}
                  className={`rounded-xl overflow-hidden text-xs cursor-pointer ${fin.status === 'completed' ? 'ring-2 ring-yellow-500' : ''}`}
                  style={{ background: 'var(--color-surface-2)', border: '1px solid var(--color-border)' }}>
                  ...
                  {[{team: getTeam(fin.teamAId), score: fin.scoreA, winner: fin.winner===fin.teamAId}, {team: getTeam(fin.teamBId), score: fin.scoreB, winner: fin.winner===fin.teamBId}].map((row, i) => (
                    <div key={i} className={`flex items-center justify-between px-3 py-2 ${i === 0 ? 'border-b' : ''} ${row.winner ? 'font-bold' : ''}`}
                      ...
                      <div className="flex items-center gap-1.5">
                        <span className="w-5 h-5 rounded text-white text-[9px] font-bold flex items-center justify-center" style={{ background: row.team?.color ?? '#64748b' }}>{row.team?.logo ?? '?'}</span>
                        <span style={{ color: row.winner ? row.team?.color : 'var(--color-text)' }}>{row.team?.name ?? 'TBD'}</span>
                      </div>
                      <span className="font-bold ml-2 tabular-nums" style={{ color: row.winner ? row.team?.color : 'var(--color-text)' }}>
        ...
        {champion && (
          <>
            <div className="flex items-center self-center sm:mt-8">
              <div className="w-8 h-px hidden sm:block" style={{ background: 'linear-gradient(90deg,#fbbf24,#F97316)' }} />
              <div className="w-0 h-0 border-t-4 border-t-transparent border-b-4 border-b-transparent border-l-8 hidden sm:block" style={{ borderLeftColor: '#fbbf24' }} />
            </div>
            <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: 'spring' }}
              className="flex-shrink-0 rounded-2xl p-4 text-center champion-glow self-center sm:mt-8"
              style={{ background: 'rgba(234,179,8,0.12)', border: '2px solid rgba(234,179,8,0.4)', minWidth: 120 }}>
              ...
              <div className="font-bold text-xs text-yellow-500">{champion.name}</div>
              <div className="text-[10px] text-muted">🏆 Champion</div>
            </motion.div>
          </>
        )}
      </div>
    ...
    <Layout>
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="font-display font-bold text-3xl gradient-text">Sequence Groups</h1>
        </div>
        <p className="text-muted text-sm">Group stage · Top 2 from each group advance to playoffs</p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
        {[
          ...
          { value: seqMatches.filter(m=>m.status==='upcoming').length, label: 'Upcoming', color: '#F97316' },
        ].map((s) => (
      ...
      {liveSeq.length > 0 && (
        <div className="mb-8">
          <h2 className="font-semibold mb-3 flex items-center gap-2" style={{ color: 'var(--color-text)' }}>
      ...
      {/* Playoff Bracket */}
      <div className="mb-8">
        <h2 className="font-semibold text-lg mb-4" style={{ color: 'var(--color-text)' }}>🏆 Sequence Playoffs</h2>
      ...
      {playoffMatches.length > 0 && (
        <div className="mb-8">
          <h2 className="font-semibold text-lg mb-4" style={{ color: 'var(--color-text)' }}>Playoff Matches</h2>
          <div className="grid sm:grid-cols-2 gap-4">
      ...
      <div>
        <h2 className="font-semibold text-lg mb-4" style={{ color: 'var(--color-text)' }}>Group Stage Results</h2>
      ...
      {/* Upcoming */}
      <div className="mt-8">
        <h2 className="font-semibold text-lg mb-4" style={{ color: 'var(--color-text)' }}>Upcoming</h2>
        <div className="grid sm:grid-cols-2 gap-3">
          {seqMatches.filter(m=>m.status==='upcoming').map((m) => (
            <MatchCard key={m.id} match={m} teamA={getTeam(m.teamAId)!} teamB={getTeam(m.teamBId)!} compact />
          ))}
          {seqMatches.filter(m=>m.status==='upcoming').length === 0 && (
            <div className="col-span-2 surface rounded-xl p-6 text-center text-muted text-sm">No upcoming sequence matches</div>
          )}
        </div>
      </div>
    </Layout>
