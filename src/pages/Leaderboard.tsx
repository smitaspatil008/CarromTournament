          ...
          {[1, 0, 2].map((idx) => {
            ...
            return (
              ...
              >
                ...
                <div className="text-[10px] sm:text-xs font-semibold text-center leading-tight text-gray-900 line-clamp-2">{t.name}</div>
                <div className="text-[9px] sm:text-[10px] text-gray-500">{t.points} pts</div>
                <div className="mt-1 sm:mt-2 rounded-t-xl w-full flex items-end justify-center font-bold text-white text-xs sm:text-sm"
                  style={{ height: heights[idx], background: idx===0?'#eab308':idx===1?'#2563EB':'#6b7280' }}>
                  #{idx + 1}
                </div>
              </motion.div>
            );
        ...
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50">
              <tr>
                <th className="text-left pl-3 pr-1 py-3 text-xs font-semibold text-gray-500 w-8">#</th>
                <th className="text-left px-1 py-3 text-xs font-semibold text-gray-500">Team</th>
                <th className="text-center px-1 py-3 text-xs font-semibold text-gray-500 w-8">W</th>
                <th className="text-center px-1 py-3 text-xs font-semibold text-gray-500 w-8">L</th>
                <th className="text-center px-1 py-3 text-xs font-semibold text-gray-500 w-10">Pts</th>
                <th className="text-center px-1 py-3 text-xs font-semibold text-gray-500 hidden sm:table-cell">Status</th>
              </tr>
            </thead>
            <tbody>
              {gameTeams.map((t, i) => (
                <motion.tr key={t.id}
                  ...
                  <td className="px-1 py-3">
                    <div className="flex items-center gap-1.5">
                      <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-lg flex-shrink-0 flex items-center justify-center text-white text-[10px] sm:text-xs font-bold" style={{ background: t.color }}>{t.logo}</div>
                      <div className="font-medium text-xs text-gray-900 truncate">{t.name}</div>
                    </div>
                  </td>
                  <td className="text-center px-1 py-3 font-semibold text-green-500">{t.wins}</td>
                  <td className="text-center px-1 py-3 font-semibold text-red-400">{t.losses}</td>
                  <td className="text-center px-1 py-3">
                    <span className="font-bold text-blue-600">{t.points}</span>
                  </td>
                  <td className="text-center px-1 py-3 hidden sm:table-cell">
